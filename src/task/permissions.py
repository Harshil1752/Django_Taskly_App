...
class IsAllowedToEditTaskElseNone(permissions.BasePermission):
    """
    Custom permissions for TaskListViewset to Only Allow members of a house access to its tasks.
    """
    def has_permission(self, request, view):
        if not request.user.is_anonymous:
...
class IsAllowedEditAttachmentElseNone(permissions.BasePermission):
